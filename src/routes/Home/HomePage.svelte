...
<div class="px-4 md:px-0 lg:px-16 text-black bg-white dark:bg-black dark:text-white">
	<div
		class="md:block px-10 lg:px-20 flex justify-between items-center bg-white text-black md:pb-8 fixed top-0 left-0 right-0 z-50 dark:bg-black dark:text-white"
	>
		...
		>
			<img class="w-20 md:w-28 lg:w-36 dark:invert" src="./Joey.png" alt="Joey logo" />
		</div>
		<!-- Header start -->
		<header data-sal-delay="300" data-sal="slide-up" data-sal-duration="500" class="relative">
			<nav class="flex justify-center md:block">
				...
				<button
					id="menuToggle"
					class="md:hidden z-20 bg-black text-white p-2 rounded-full dark:bg-white dark:text-black"
					on:click={toggleMenu}
				>
					<svg
				...
				<ul
					class="{isMenuOpen
						? 'flex'
						: 'translate-x-full'} md:translate-x-0 md:opacity-100 transition-all duration-300 ease-in-out fixed md:relative top-0 right-0 h-fit w-64 md:w-fit md:mx-auto flex flex-col md:flex-row p-4 md:p-1 space-y-2 md:space-y-0 md:space-x-4 items-start justify-start md:items-center bg-black text-white md:bg-black md:rounded-full z-40 rounded-bl-2xl dark:bg-white dark:text-black"
				>
					...
					{#each ['Home', 'Projects', 'About', 'Contact'] as tab}
						...
						<li
							class="w-full md:w-auto text-center {activeTab === tab
								...
								: 'bg-black text-white dark:bg-white dark:text-black'} px-4 py-2 rounded-full font-medium cursor-pointer transition-all duration-300 ease-in-out"
	...
	>
		<h1
			class="uppercase absolute left-[60%] md:left-[70%] px-6 lg:left-[75%] pb-[5%] pl-[3%] bottom-[2rem] text-lg lg:text-xl font-bold flex items-center gap-2 animate-bouncesmall"
		>
			Scroll Down
			<svg
				width="20"
